...
Usage (paper, default):
    ...
    python scripts/test_round_trip_trade.py --wait 10 --direction long
...
def resolve_client_id(ibkr_cfg: dict, override: int | None) -> int:
    ...
    if raw.isdigit():
        return int(raw)
    # Default 99 so this test does not clash with the main bot (often client_id=1)
    cfg_id = ibkr_cfg.get("connection", {}).get("client_id")
    if cfg_id is not None:
        return int(cfg_id) + 10
    return 99
...
async def place_and_fill(ib: IB, contract, action: str, qty: int, tracker: IbErrorTracker):
    print(f"\n>>> Placing MARKET {action} x{qty} ...")
    order = _make_market_order(action, qty)
    trade = ib.placeOrder(contract, order)
    await wait_for_fill(trade, tracker)
    px = trade.orderStatus.avgFillPrice
    print(f"[OK] Filled {action} x{qty} @ {px:.2f}")
    return float(px)
...
async def run_test(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        entry_px = await place_and_fill(ib, contract, open_action, args.contracts, tracker)
        print(f"\n--- Holding for {args.wait} seconds ---")
        await asyncio.sleep(args.wait)
        exit_px = await place_and_fill(ib, contract, close_action, args.contracts, tracker)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--wait", type=int, default=10, help="Seconds to hold before closing")
